...
delData = () =>{
    ...
    .then(responseData => {
        this.resetIdView();
        this.disableFrm(true);
        // Aquí e.target.dataset.ed no está disponible directamente, necesitas el evento o una lógica diferente si quieres controlar botones después de la eliminación.
        // this.ctrlBtn(e.target.dataset.ed); 
        // Hacer algo con la respuesta exitosa si es necesario
    })
    .catch(error => {
...
saveData = () =>{
        ...
        document.querySelector('#btnGuardar').addEventListener("click",(e) =>{
            const datos = Object.fromEntries(new FormData(frmRegistro).entries());
            postBranches(datos) // ¡CORREGIDO! Antes era postContacts
            .then(response => {
                // Verificar si la solicitud fue exitosa (código de respuesta en el rango 200)
                if (response.ok) {
                    return response.json(); // Devolver la respuesta como JSON
